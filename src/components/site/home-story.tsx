...
export function HomeStory() {
  return <>
    ...
    <section className="audit-principles section" id="how-it-works">
      <div className="shell"><p className="eyebrow"><span /> 01 / Start with the free audit</p><h2>Find your next best move, not just your total.</h2><p className="story-lead">See which bill deserves attention first, where tools may overlap and which costs grow with your team. Tell us what gets in the way, and get a clear starting point for fixing it.</p><div className="principle-grid">{runPlan.map((item) => <article key={item.letter}><span>{item.letter}</span><h3>{item.title}</h3><p>{item.text}</p></article>)}</div><div className="story-action"><Link className="button" href="/#audit">Run My Free Audit <span aria-hidden="true">↗</span></Link><p>Start with 1 to 3 tools. Skip what you do not know. See your results before sharing your email.</p></div><p className="story-note">The audit gives you a starting point, not a savings promise or a quote to build something new.</p></div>
    </section>
