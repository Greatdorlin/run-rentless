...
export function WorkflowQuestion({ tool, features, index, count, update, back, next, finish }: {
  ...
  return <div className="audit-panel guided-question">
    ...
    {question === 0 && <fieldset><legend><h1>Does it make work easier?</h1></legend><p>Does your team have to find other ways to get the job done?</p><div className="guided-options">{[{ value: "Rarely", label: "It usually does the job" }, { value: "Sometimes", label: "We sometimes work around it" }, { value: "Constantly", label: "We constantly work around it" }].map(({ value, label }) => <button key={value} type="button" aria-pressed={tool.workaround === value} className={tool.workaround === value ? "selected" : ""} onClick={() => update({ workaround: tool.workaround === value ? "Not answered" : value })}>{label}</button>)}</div></fieldset>}
    {question === 1 && <fieldset><legend><h1>What do you use it for?</h1></legend><p>Choose the jobs it does. This helps us spot tools doing the same job.</p><div className="check-grid">{features.map((feature) => <label key={feature}><input type="checkbox" checked={tool.features.includes(feature)} onChange={() => update({ features: tool.features.includes(feature) ? tool.features.filter((item) => item !== feature) : [...tool.features, feature] })} />{feature}</label>)}</div></fieldset>}
    {question === 2 && <label className="question"><h1>What is the one thing you wish it did better?</h1><p>Tell us about a task that takes too long or a feature you need.</p><textarea autoFocus maxLength={600} value={tool.wish} onChange={(event) => update({ wish: event.target.value })} placeholder={tool.category === "Scheduling" ? "For example: Assign bookings to the right branch automatically." : "For example: Remind the right person when a customer has not heard back."} /></label>}
